...
> 第三輪更新優先：MA20優先已由使用者定案；補至550%的to550保存未取整值，整元入金另外顯示。實務壓測改用查核後的當期保證金；真實Firestore隔離驗收已完成，但cron與正式發布仍未啟動。完整證據見[第三輪驗證](defense-round3-validation.md)。以下第二輪待確認或尚未實寫敘述僅代表該輪當時狀態。
...
決策時總策略權益＝持倉先MTM後的期貨帳戶權益＋場外資金；目標名目值＝動態總權益×目標倍數，實際曝險＝期貨名目值/動態總權益。2,000,000只是Forward期初本金與績效基準，不得作為永久配口分母。乘數 TX/TXF=200、MTX/MXF=50、TMF=10。收盤 < MA60 且 MA60 < 第20個交易日前 MA60 才確認空頭；至少需要80筆有效日收盤。計算不先四捨五入。
