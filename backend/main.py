...
def fetch_price():
    try:
        ...
        try:
            ...
            txf_price = txf_result["meta"].get("regularMarketPrice") or txf_result["indicators"]["quote"][0]["close"][-1]
            txf_price = round(txf_price, 0)
        except:
...
def dashboard():
    ...
    if 0 < dist <= 300:
        send_telegram(f"⚠️ 台指期警戒！\n現價 {price}，距硬底線 {dynamic_floor} 僅剩 {round(dist,0)} 點！")
    if price <= dynamic_floor:
        send_telegram(f"🚨 緊急！台指期跌破硬底線！\n現價 {price} ≤ {dynamic_floor}，依計劃全數清倉！")
    ...
    return {
        ...
        "adong_signal": {"candle": candle, "action": "今日收紅 → 留倉" if candle=="green" else "今日收黑 → 當日加碼單出清"},
        "checklist": {
            "below_ma5": price < market["ma5"],
            "near_floor": 0 < (price - dynamic_floor) <= 300,
            "at_floor": price <= dynamic_floor
        }
    }
...
def add_position(pos: PositionCreate):
    conn = get_db()
    cur = conn.execute(
        "INSERT INTO positions (date,type,lots,entry_price,note,status) VALUES (?,?,?,?,?,'open')",
...
def close_position(pos_id: int, data: PartialClose):
    ...
    conn.execute(
        "INSERT INTO realized_pnl (date,lots,entry_price,exit_price,pnl_twd,note) VALUES (?,?,?,?,?,?)",
...
def add_log(log: DailyLog):
    conn = get_db()
    conn.execute(
        "INSERT INTO daily_logs (date,close_price,ma5,ma20,ma60,lots,note) VALUES (?,?,?,?,?,?,?)",
...
def save_snapshot():
    ...
    today = str(datetime.now().date())
    conn.execute(
        "INSERT OR REPLACE INTO pnl_snapshots (date,close_price,total_lots,unrealized_pnl,avg_cost,dynamic_floor) VALUES (?,?,?,?,?,?)",
    ...
    for s in rows:
        ...
        if price:
            if s["alert_high"] and price >= s["alert_high"]:
                send_telegram(f"🚀 <b>{s['symbol']}</b> 突破提醒！\n現價 {price} ≥ {s['alert_high']}")
            if s["alert_low"] and price <= s["alert_low"]:
                send_telegram(f"⚠️ <b>{s['symbol']}</b> 跌破提醒！\n現價 {price} ≤ {s['alert_low']}")
